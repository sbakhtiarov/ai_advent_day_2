...
class JsonFileWorkingMemoryStore(
    ...
    override fun load(): WorkingMemoryState? {
        ...
        val snapshot = runCatching {
            json.decodeFromString<WorkingMemorySnapshotDto>(fileContents)
    ...
    companion object {
        ...
        fun fromDefaultLocation(json: Json = defaultJson()): JsonFileWorkingMemoryStore? {
            val homeDirectory = readHomeDirectory() ?: return null
            val normalizedHome = homeDirectory.trimEnd('/')
            return JsonFileWorkingMemoryStore(
                filePath = "$normalizedHome/.kotlin-agent-cli/working-memory.json",
                json = json,
            )
        }
        ...
        private fun writeTextFile(path: String, text: String) {
            ...
            try {
                if (fputs(text, file) < 0) {
                    throw IllegalStateException("Unable to write working memory file '$path'.")
                }
            } finally {
                fclose(file)
            }
    ...
    private fun deleteFileIfExists(path: String) {
        ...
        throw IllegalStateException("Unable to remove working memory file '$path'.")
    }
}
